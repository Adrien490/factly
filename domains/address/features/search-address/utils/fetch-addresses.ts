...
import { SearchAddressParams, SearchAddressReturn } from "../types";
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 400) {
					...
					console.error(
						"[FETCH_ADDRESSES] Détails de l'erreur 400:",
						errorText
					);
			...
			if (fetchError instanceof Error && fetchError.name === "AbortError") {
				console.error(
					"[FETCH_ADDRESSES] Timeout de la requête API après 15 secondes"
				);
				return {
					results: [],
					metadata: {
			...
			// Propager l'erreur pour qu'elle soit traitée par le catch externe
			throw fetchError;
		}
	} catch (error) {
