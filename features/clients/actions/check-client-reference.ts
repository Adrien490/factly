...
export type CheckClientReferenceResponse = {
	reference: string;
	exists: boolean;
};

export default async function checkClientReference(
	_: ServerActionState<unknown, typeof checkClientReferenceSchema>,
	formData: FormData
): Promise<
	ServerActionState<
...
> {
	try {
		...
		const validation = checkClientReferenceSchema.safeParse(rawData);
